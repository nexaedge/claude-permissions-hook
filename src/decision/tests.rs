...
fn file_input(tool: &str, mode: &str, tool_input: serde_json::Value) -> HookInput {
    make_input(tool, mode, tool_input)
}

/// Extract decision from evaluate result.
fn file_decision(input: &HookInput, config: &Config) -> Decision {
    evaluate(input, Some(config))
        .unwrap()
        .hook_specific_output
        .permission_decision
...
/// Extract reason from evaluate result.
fn file_reason(input: &HookInput, config: &Config) -> String {
    evaluate(input, Some(config))
        .unwrap()
        .hook_specific_output
        .permission_decision_reason
...
// ---- Reason string tests for file tools ----
...
fn file_config_with_both_bash_and_files() {
    ...
    let bash_in = bash_input("git status", "default");
    assert_eq!(
