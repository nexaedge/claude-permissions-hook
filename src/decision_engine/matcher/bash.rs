...
use crate::domain::rule::bash::{ArgumentPattern, BashRule};
use crate::domain::CommandSegment;

/// Check whether a parsed command segment satisfies a rule's conditions.
///
/// All non-empty conditions must pass (AND semantics).
/// Empty conditions = unconditional match (backwards compat with v0.2.0).
pub(crate) fn matches(rule: &BashRule, segment: &CommandSegment) -> bool {
    program_matches(rule, segment)
        && flags_match(rule, segment)
        && subcommand_matches(rule, segment)
        && positionals_match(rule, segment)
        && required_arguments_match(rule, segment)
        && subcommands_match(rule, segment)
}

/// Program name must match (both sides are already basename-normalized).
fn program_matches(rule: &BashRule, segment: &CommandSegment) -> bool {
    rule.program == segment.program
}

/// Required flags: ALL must be present. Optional flags: if non-empty, ANY one must be present.
fn flags_match(rule: &BashRule, segment: &CommandSegment) -> bool {
    ...
    if !rule.conditions.optional_flags.is_empty()
        && !rule
            .conditions
            .optional_flags
            .iter()
...
fn subcommand_matches(rule: &BashRule, segment: &CommandSegment) -> bool {
    if rule.conditions.subcommand.is_empty() {
        return true;
    }
    let (_, actual_positionals) = classify_args(&segment.args);
    if actual_positionals.len() < rule.conditions.subcommand.len() {
        return false;
    }
    rule.conditions
        .subcommand
...
fn positionals_match(rule: &BashRule, segment: &CommandSegment) -> bool {
    if rule.conditions.positionals.is_empty() {
        return true;
    }
    let (_, actual_positionals) = classify_args(&segment.args);
    rule.conditions.positionals.iter().all(|pattern| {
        actual_positionals
            .iter()
...
fn required_arguments_match(rule: &BashRule, segment: &CommandSegment) -> bool {
    rule.conditions
        .required_arguments
        .iter()
...
fn subcommands_match(rule: &BashRule, segment: &CommandSegment) -> bool {
    if rule.conditions.subcommands.is_empty() {
        return true;
    }
    let (_, actual_positionals) = classify_args(&segment.args);
    rule.conditions.subcommands.iter().any(|chain| {
...
mod tests {
    use super::*;
    use crate::domain::rule::bash::{compile_glob, BashConditions};
    ...
    fn rule_required_arguments(program: &str, pairs: &[(&str, &str)]) -> BashRule {
        ...
        for (flag, value_pattern) in pairs {
            conditions
                .required_arguments
                .push(crate::domain::rule::bash::ArgumentPattern {
                    flag: flag.to_string(),
    ...
    fn match_empty_conditions_with_flags_and_positionals() {
        assert!(matches(&rule("rm"), &seg("rm", &["-r", "-f", "/tmp"])));
    }

    #[test]
    fn match_empty_conditions_with_no_args() {
        assert!(matches(&rule("rm"), &seg("rm", &[])));
    }

    #[test]
    fn match_empty_conditions_with_subcommand_and_flag() {
        assert!(matches(&rule("git"), &seg("git", &["push", "--force"])));
    }
    ...
    fn no_match_optional_flags_wrong_flag() {
        let r = rule_optional_flags("rm", &["-r", "-f"]);
        assert!(!matches(&r, &seg("rm", &["--verbose", "/tmp"])));
    }
    ...
    fn match_subcommand_exact() {
        let r = rule_subcommand("git", &["push", "origin"]);
        assert!(matches(&r, &seg("git", &["push", "origin"])));
    }

    #[test]
    fn no_match_subcommand_wrong_second() {
        let r = rule_subcommand("git", &["push", "origin"]);
        assert!(!matches(&r, &seg("git", &["push", "main"])));
    }

    #[test]
    fn no_match_subcommand_too_few_args() {
        let r = rule_subcommand("git", &["push", "origin"]);
        assert!(!matches(&r, &seg("git", &["push"])));
    }

    #[test]
    fn no_match_subcommand_wrong_order() {
        let r = rule_subcommand("git", &["push", "origin"]);
        assert!(!matches(&r, &seg("git", &["origin", "push"])));
    }
    ...
    fn match_subcommand_with_interleaved_flags() {
        ...
        assert!(matches(
            &r,
            &seg("git", &["push", "origin", "--force", "main"])
        ));
    }
    ...
    fn match_subcommand_empty_always_matches() {
        ...
        assert!(matches(&r, &seg("rm", &["/tmp", "file.txt"])));
    }

    #[test]
    fn match_subcommand_empty_no_args() {
        let r = rule_subcommand("rm", &[]);
        assert!(matches(&r, &seg("rm", &[])));
    }
    ...
    fn no_match_positional_glob_nested() {
        let r = rule_positionals("rm", &["/*"]);
        assert!(!matches(&r, &seg("rm", &["/home/user"])));
    }

    #[test]
    fn no_match_positional_no_leading_slash() {
        let r = rule_positionals("rm", &["/*"]);
        assert!(!matches(&r, &seg("rm", &["file.txt"])));
    }
    ...
    fn match_positional_any_order() {
        ...
        assert!(matches(&r, &seg("git", &["push", "main", "origin"])));
    }
    ...
    fn match_subcommand_and_positional() {
        ...
        assert!(matches(&r, &seg("claude", &["mcp", "add", "linear"])));
    }

    #[test]
    fn no_match_subcommand_ok_positional_missing() {
        let r = rule_subcommand_and_positionals("claude", &["mcp", "add"], &["linear"]);
        assert!(!matches(&r, &seg("claude", &["mcp", "add", "github"])));
    }

    #[test]
    fn no_match_subcommand_wrong_order_with_positional() {
        let r = rule_subcommand_and_positionals("claude", &["mcp", "add"], &["linear"]);
        assert!(!matches(&r, &seg("claude", &["add", "mcp", "linear"])));
    }
    ...
    fn match_subcommand_and_positional_extra_args() {
        ...
        assert!(matches(
            &r,
            &seg("claude", &["mcp", "add", "linear", "extra"])
        ));
    }
    ...
    fn match_subcommands_first_chain() {
        ...
        assert!(matches(&r, &seg("git", &["status"])));
    }

    #[test]
    fn match_subcommands_second_chain_with_flag() {
        let r = rule_subcommands("git", &[&["status"], &["log"], &["diff"]]);
        assert!(matches(&r, &seg("git", &["log", "--oneline"])));
    }

    #[test]
    fn match_subcommands_third_chain_with_extra_args() {
        let r = rule_subcommands("git", &[&["status"], &["log"], &["diff"]]);
        assert!(matches(&r, &seg("git", &["diff", "HEAD~1"])));
    }
    ...
    fn no_match_subcommands_wrong_command() {
        let r = rule_subcommands("git", &[&["status"], &["log"], &["diff"]]);
        assert!(!matches(&r, &seg("git", &["rebase", "main"])));
    }
    ...
    fn match_subcommands_multi_word_flag_before() {
        ...
        assert!(matches(
            &r,
            &seg("git", &["--force", "push", "origin", "main"])
        ));
    }
    ...
    fn match_subcommands_mcp_add() {
        ...
        assert!(matches(&r, &seg("claude", &["mcp", "add", "server"])));
    }
    ...
    fn match_subcommands_mcp_remove() {
        ...
        assert!(matches(&r, &seg("claude", &["mcp", "remove", "server"])));
    }
    ...
    fn match_subcommands_mcp_list_exact() {
        ...
        assert!(matches(&r, &seg("claude", &["mcp", "list"])));
    }
    ...
    fn no_match_subcommands_mcp_wrong_sub() {
        ...
        assert!(!matches(&r, &seg("claude", &["config"])));
    }
    ...
    fn no_match_subcommands_mcp_wrong_order() {
        ...
        assert!(!matches(&r, &seg("claude", &["add", "mcp"])));
    }
    ...
    fn no_match_flags_ok_subcommands_wrong() {
        let r = rule_subcommands_with_flags("git", &[&["push"]], &["--force"]);
        assert!(!matches(&r, &seg("git", &["pull", "--force"])));
    }
    ...
    fn match_required_argument_next_arg() {
        ...
        assert!(matches(
            &r,
            &seg("curl", &["--upload-file", "data.txt", "url"])
        ));
    }
    ...
    fn no_match_required_argument_flag_not_present() {
        let r = rule_required_arguments("curl", &[("--upload-file", "*")]);
        assert!(!matches(&r, &seg("curl", &["url"])));
    }

    #[test]
    fn no_match_required_argument_flag_no_value() {
        let r = rule_required_arguments("curl", &[("--upload-file", "*")]);
        assert!(!matches(&r, &seg("curl", &["--upload-file"])));
    }
    ...
    fn match_program_basename_absolute_path() {
        assert!(matches(&rule("rm"), &seg("/usr/bin/rm", &["-r"])));
    }

    #[test]
    fn no_match_program_different_name() {
        assert!(!matches(&rule("rm"), &seg("mv", &["-f"])));
    }
    ...
    fn no_match_positionals_ok_subcommands_wrong() {
        let r = rule_subcommands_with_positionals("git", &[&["push"]], &["origin"]);
        assert!(!matches(&r, &seg("git", &["pull", "origin"])));
    }
